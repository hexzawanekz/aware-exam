...
const AdminPositions = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingPosition) {
        await apiService.updatePosition(editingPosition.id, positionData);
      } else {
  ...
  const editPosition = (position) => {
    ...
    setFormData({
      ...
      programming_language_id:
        position.programming_language_id?.toString() || "",
    });
    setShowForm(true);
  };
    ...
    <div className="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:gap-3">
          ...
          >
            {showForm ? t("common.cancel") : `+ ${t("admin.add_position")}`}
          </button>
        </div>
      </div>
            ...
            <form onSubmit={handleSubmit}>
              <div className="mb-5.5">
                ...
                <input
                  ...
                  placeholder={t("admin.position_name_placeholder")}
              ...
              <div className="mb-5.5">
                <label className="mb-3 block text-sm font-medium text-black dark:text-white">
                  {t("admin.programming_language")} ({t("admin.optional")})
                </label>
                <select
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="px-4 py-6 md:px-6 xl:px-7.5">
          <h4 className="text-xl font-semibold text-black dark:text-white">
            {t("admin.positions_list")}
          </h4>
        </div>

        <div className="grid grid-cols-6 border-t border-stroke px-4 py-4.5 dark:border-strokedark sm:grid-cols-8 md:px-6 2xl:px-7.5">
          <div className="col-span-2 flex items-center">
            <p className="font-medium">{t("admin.position_name")}</p>
          </div>
          <div className="col-span-2 hidden items-center sm:flex">
            <p className="font-medium">{t("admin.department")}</p>
          </div>
          <div className="col-span-2 hidden items-center sm:flex">
            ...
            >
              ...
              <div className="col-span-2 hidden items-center sm:flex">
                <p className="text-sm text-black dark:text-white">
                  {position.department?.name || t("admin.no_department")}
                  {position.department?.company && (
                    <span className="text-xs text-body dark:text-bodydark block">
              ...
              <div className="col-span-2 hidden items-center sm:flex">
                <p className="text-sm text-black dark:text-white">
                  {position.programming_language?.name ||
                    t("admin.no_programming_language")}
                </p>
              </div>
